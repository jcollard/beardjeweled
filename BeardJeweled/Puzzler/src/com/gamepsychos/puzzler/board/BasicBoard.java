...
public class BasicBoard implements Board {
	...
	/**
	 * Creates a {@code BasicBoard} that is 8x6 and specifies the
	...
	private final MoveResult clearBoard(){
		...
		if (locations.isEmpty()) {
			...
			return new BasicMoveResult(locations, null, drops);
		}
		...
		return new BasicMoveResult(locations, new FollowUpMove(), drops);
	}

	private final Set<Drop> findDrops() {
		Set<Drop> drops = new HashSet<Drop>();
		for (int r = rows; r > 0; r--) { // Start at the bottom and go up.
			for (int c = 0; c < cols; c++) {
				if (piece[r][c] == null) {
	...
	private final Drop executeDrop(int row, int col) {
		...
		for (int r = row; r >= 0; r--) {
			if (piece[row][col] != null) {
				Location start = Location.getLocation(row, col);
	...
	private final class IgnoreMove implements Move {
		...
		public MoveResult move() {
			...
			Set<Drop> drops = Collections.emptySet();
			return new BasicMoveResult(emptySet, null, drops);
		}
